...
describe('GET /api/catalog/encounter', () => {
  ...
  it('returns service and fallback error messages', async () => {
    listMock.mockRejectedValueOnce(new Error('Encounter API unavailable'));

    const errorResponse = await GET({ nextUrl: { searchParams: new URLSearchParams() } } as never);
    expect(errorResponse.status).toBe(500);
    await expect(errorResponse.json()).resolves.toEqual({ message: 'Encounter API unavailable' });
    ...
    await expect(fallbackResponse.json()).resolves.toEqual({ message: 'Could not load Pokemon Encounters.' });
  });
});
