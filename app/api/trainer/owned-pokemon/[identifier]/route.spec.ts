...
describe('GET /api/trainer/owned-pokemon/[identifier]', () => {
  beforeEach(() => {
    jest.clearAllMocks();
    getServerSessionMock.mockResolvedValue({ isAuthenticated: true, token: 'token' });
  });

  it('delegates identifier to the trainer service owned pokemon', async () => {
  ...
  it('uses the default detail error message for unknown failures', async () => {
    ...
    expect(response.status).toBe(500);
    expect(json).toEqual({ message: 'Could not load Owned Pokemon detail.' });
  });
});
