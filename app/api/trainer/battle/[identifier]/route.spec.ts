...
jest.mock('@/app/ui', () => ({
  trainerService: jest.fn(() => ({ battle: { detail: detailMock } })),
}));

describe('GET /api/trainer/battle/[identifier]', () => {
  beforeEach(() => {
    jest.clearAllMocks();
    getServerSessionMock.mockResolvedValue({ isAuthenticated: true, token: 'token' });
  });

  it('delegates identifier to the trainer service battle', async () => {
  ...
  it('uses the default detail error message for unknown failures', async () => {
    ...
    expect(response.status).toBe(500);
    expect(json).toEqual({ message: 'Could not load Battle detail.' });
  });
});
