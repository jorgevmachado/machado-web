import { PokemonMoveBffService } from './bffService';

class TestPokemonMoveBffService extends PokemonMoveBffService {
...
describe('PokemonMoveBffService', () => {
  it('sets domain, pathUrl and baseUrl in constructor', () => {
    const service = new TestPokemonMoveBffService('pokemon/move');

    expect(service.getDomain()).toBe('pokemon.move');
    expect(service.getPathUrl()).toBe('pokemon/move');
  ...
  it('builds detail endpoint and domain i18n message on fetchOne', async () => {
    ...
    expect(getSpy).toHaveBeenCalledWith('pokemon/move/name');
    expect(response).toEqual({
      data: { id: 1, name: 'name' },
      error: false,
      status: 200,
      message: 'OK',
      i18nMessage: 'pokemon.move.detail.loadError',
    });
  });
  ...
  it('builds list endpoint and domain i18n message on fetchAll', async () => {
    ...
    expect(getSpy).toHaveBeenCalledWith('pokemon/move?page=1&limit=10&name=name');
    expect(response).toEqual({
      data,
      error: false,
      status: 200,
      message: 'OK',
      i18nMessage: 'pokemon.move.list.loadError',
    });
  });
});
