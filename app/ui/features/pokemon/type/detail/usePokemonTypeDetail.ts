  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      if (!response.ok || !('id' in json)) {
        const message = 'message' in json && json.message ? json.message : t('pokemon.type.detail.loadError');
      ...
      const message = error instanceof Error && error.message ? error.message : t('pokemon.type.detail.loadError');
      setState({ data: undefined, isLoading: false, errorMessage: message });
      showAlert({ type: 'error', message });
    } finally {
      stopContentLoading();
