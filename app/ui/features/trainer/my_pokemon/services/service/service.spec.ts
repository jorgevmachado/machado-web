...
describe('MyPokemonService', () => {
  ...
  it('calls the list endpoint', async () => {
    ...
    expect(fetchMock).toHaveBeenCalledWith('https://api.example.com/trainer/my-pokemon?page=1&limit=12', expect.objectContaining({
      method: 'GET',
    }));
  });
  ...
  it('calls the detail endpoint', async () => {
    ...
    expect(fetchMock).toHaveBeenCalledWith('https://api.example.com/trainer/my-pokemon/bulbasaur-1', expect.any(Object));
  });
  ...
  it('calls the create endpoint', async () => {
    ...
    expect(fetchMock).toHaveBeenCalledWith('https://api.example.com/trainer/my-pokemon', expect.any(Object));
  });
});
