...
class TestTrainerBffService extends TrainerBffService {
...
describe('TrainerBffService', () => {
  it('sets domain, pathUrl and baseUrl in constructor', () => {
    const service = new TestTrainerBffService('trainer');

    expect(service.getDomain()).toBe('trainer');
    expect(service.getPathUrl()).toBe('trainer');
  ...
  it('builds detail endpoint and domain i18n message on fetchOne', async () => {
    ...
    expect(getSpy).toHaveBeenCalledWith('trainer/name');
    expect(response).toEqual({
      data: { id: 1, name: 'name' },
      error: false,
      status: 200,
      message: 'OK',
      i18nMessage: 'trainer.detail.loadError',
    });
  });
  ...
  it('builds list endpoint and domain i18n message on fetchAll', async () => {
    ...
    expect(getSpy).toHaveBeenCalledWith('trainer?page=1&limit=10&name=name');
    expect(response).toEqual({
      data,
      error: false,
      status: 200,
      message: 'OK',
      i18nMessage: 'trainer.list.loadError',
    });
  });
});
