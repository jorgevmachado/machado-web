...
describe('PokedexService', () => {
  ...
  it('calls the list endpoint', async () => {
    ...
    expect(fetchMock).toHaveBeenCalledWith('https://api.example.com/trainer/pokedex?page=1&limit=12', expect.objectContaining({
      method: 'GET',
    }));
  });
  ...
  it('calls the detail endpoint', async () => {
    ...
    expect(fetchMock).toHaveBeenCalledWith('https://api.example.com/trainer/pokedex/bulbasaur', expect.any(Object));
  });
  ...
  it('calls the discover endpoint', async () => {
    ...
    expect(fetchMock).toHaveBeenCalledWith('https://api.example.com/trainer/pokedex/bulbasaur/discover', expect.any(Object));
  });
});
